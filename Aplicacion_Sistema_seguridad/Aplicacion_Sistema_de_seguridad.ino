...
void setup() {
  ...
  lcd.begin(16, 2);
  //Solicitar que se ingrese una contraseña
  lcd.setCursor(2, 0);
  lcd.print("Bienvenido");
  delay(500);
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Ingrese una KEY:");
}
...
// Variable para almacenar las contraseñas
int keySave = 0;
int keyIn = 0;
int botonGuardar = 0;
void loop() {
  ...
  barridoTeclado();
  valor();
  //Boton asigando para guardar. Tecla A
  if (numero == 15 && botonGuardar == 0) {
    botonGuardar = 1;
  }
  //Mostrar el codigo de 4 digitos antes de guardar
  if (botonGuardar == 0) {
    lcd.setCursor(0, 1);
    lcd.print(fourdig);
    lcd.print("   ");
  ...
  //Boton para volver a ctivar la alarma
  int botonActivar = numero;
  if (botonActivar == 14) {
    lcd.clear();
    lcd.setCursor(1, 0);
    lcd.print("Seguridad ON");
